...
**결론 및 한계**
<br>
최종 결과물은 시각적으론 깔끔해 보이지만 직접 들었을 때 여전히 노이즈가 확인됩니다. Amplitude Spectrogram만을 예측하는 모델의 특성상, 학습 성과와 무관하게 PESQ 스코어를 높이는 데에 명확한 한계를 가지고 있습니다. 반면 본 모델과 달리 최신 노이즈 제거 모델을 살펴보면 Phase Spectrogram을 모델 인풋에서 획득하지 않는 방식도 존재합니다. 따라서 Phase Spectrogram을 Amplitude Spectrogram과 함께 예측하거나, 보정된 형태를 사용할 수 있는 방법을 쓰거나, Waveforms 그 자체에서 모델 예측을 하는 등의 시도를 해볼 수 있을 것입니다. 다만 이러한 방식은 많은 파라미터 수를 요구할 가능성이 높습니다. 저희가 구현한 모델은 인퍼런스 속도가 매우 빠르기 때문에 쉽게 구현할 수 있고, 실시간 환경에도 적용해볼 수 있는 모델이란 점에서 의의가 있다고 생각합니다. 
<br>
모델 학습 전, 오디오의 전처리 또한 중요한 문제입니다. 본 프로젝트에선 통상적인 FFT length와 Hop_length를 바로 사용하기 보단, 직접 data를 시각화하여 실제 모델이 어떻게 정보를 받아들일 수 있을지 판단해보고 전처리 조건을 바꾸어 적용시켰습니다. 샘플레이트 환경은 16000으로 주어졌습니다. Nyquist 이론에 따르면 샘플레이트가 16000일 때 표현할 수 있는 최대 주파수는 8000hz 까지입니다. 하지만 이는 사람의 음성이 담고 있는 정보를 전부 표현하기에는 여전히 부족한 범위입니다. 이를 44100까지 높여 가청 주파수를 모두 표현할 수 있도록 훈련한다면 일상적인 대화를 넘어서 보컬 등 다양한 분야에 사용이 가능해질 것입니다.
<br>
<br>
저희는 MSE LOSS를 사용하였으나, 미분가능한 PESQ나 si sdr, LMS 등 다양한 로스가 존재합니다. MSE LOSS는 현재 음성 분야에선 한계가 지적되어 최신 모델에선 다른 Loss를 사용하는 추세인 것을 확인하였으며 성능적 지표가 비교적 떨어지는 것을 확인했습니다[[9]](#9-hwang-seo-rim-et-al-performance-comparison-evaluation-of-speech-enhancement-using-various-loss-functions-the-journal-of-the-acoustical-society-of-korea-vol-40-no-2-mar-2021-pp-17682-httpsdoiorg107776ask2021402176). Loss를 바꾸는 것이 청취적 평가인 정성적으로도, PESQ metric의 정량적으로도 우수해질 수 있으리라 판단했습니다.
<br>
<br>
모델의 아웃풋을 이미지로 나타낼 때, 일반적으로 오른쪽 끝부분이 음성의 존재와 무관하게 지워지는 현상이 있다는 것을 파악했습니다. 왼쪽의 이미지는 모델을 통해 predict한 Amplitude Spectrogram입니다. 우측의 실제 ground truth의 3.5초 부근에서 엄연히 음성 신호가 존재함을 볼 수 있으나, 왼쪽의 이미지는 이를 제거한 모습입니다. 여기엔 다양한 문제가 있겠지만 깃허브의 speech enhancement 관련 여러 이슈를 찾아본 결과, 모델 외적으로는 데이터셋이 작고 쉽게 오버피팅 할 수 있는 구조일 때 음성을 제거하는 경향이 있다고 합니다. 저희는 굉장히 작은 데이터셋으로 테스트하였고, 음성과 노이즈가 다양한 조합을 갖고 있지 않습니다. 데이터셋의 한계를 극복하기 위해서 SpecAugment[[10]](#10-park-daniel-s-et-al-specaugment-a-simple-data-augmentation-method-for-automatic-speech-recognition-interspeech-2019-sept-2019-pp-261317-httpsdoiorg1021437interspeech2019-2680) 등의 오디오 데이터 어그멘테이션 기법을 사용하는 것이 좋을 것으로 생각합니다. 
<br>
또한 데이터 분류 과정에서 Validation 데이터 셋을 따로 만들지 않았는데, 위와 같은 이유입니다. 작은 데이터셋을 사용하기 때문에 트레이닝 에포크가 높지 않고 러닝 레이트 또한 낮을 때 좋은 성능을 내는 경향이 있었고, 저희가 사용했던 Loss에 대한 신뢰도가 낮기 때문입니다. 만약 SpecAugment를 사용하고 Loss를 변경한다면 Validation을 만들어 validation에 대한 pesq나 loss를 기준으로 트레이닝을 중단하는 반복문을 통해 학습시키는 것이 바람직합니다. 
<br>
<br>
